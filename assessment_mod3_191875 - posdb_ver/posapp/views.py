...
def openpos(request):
    ...
    return render (request, 'posapp/openpos.html', {'items':items})
#    return render (request, 'posapp/openpos.html')

def list_item(request):
   allitems = item.objects.all()
   return render(request, 'posapp/list_item.html', {'items':allitems})

def to_add(request):
    return render(request, 'posapp/add_item.html')

def view_orders(request):
    orders = order.objects.all().order_by('-pk')
    itemorders = order_item.objects.all()
    return render (request, 'posapp/view_orders.html', {'orders':orders, 'items':itemorders})
...
def edit_item(request, pk):
    ...
    if (request.method =="POST"):
        ...
        print(i)
        return render (request, 'posapp/edit_item.html', {'i':i})
